...
'FechaA = Year(DTPicker1.Value) & IIf(Month(DTPicker1.Value) < 10, "0" & Month(DTPicker1.Value), Month(DTPicker1.Value)) & IIf(Day(DTPicker1.Value) < 10, "0" & Day(DTPicker1.Value), Day(DTPicker1.Value))
'Fechab = Year(DTPicker2.Value) & IIf(Month(DTPicker2.Value) < 10, "0" & Month(DTPicker2.Value), Month(DTPicker2.Value)) & IIf(Day(DTPicker2.Value) < 10, "0" & Day(DTPicker2.Value), Day(DTPicker2.Value))
...
If OptSinError = True Then
    ssql = "select a.LOTE as lotecreado,a.ASIENTO as asientocreado, * from GL_JH a "
    ssql = ssql & "left outer join GL_JD b on a.BATCHID=b.BATCHNBR and a.BTCHENTRY=b.JOURNALID and a.COMPANYID=b.COMPANYID "
    ssql = ssql & " where a.COMPANYID='" & Empresa & "' and  a.ESTADO='Completo' and (a.DATEENTRY   between '" & FechaA & "' AND '" & Fechab & "' )order by a.LOTE,a.ASIENTO"
Else
    ssql = "select a.LOTE as lotecreado,a.ASIENTO as asientocreado,  * from GL_JH a "
    ssql = ssql & "left outer join GL_JD b on a.BATCHID=b.BATCHNBR and a.BTCHENTRY=b.JOURNALID and a.COMPANYID=b.COMPANYID "

    If OptError = True Then
        ssql = ssql & " where a.COMPANYID='" & Empresa & "' and  a.ESTADO='Error' and (a.DATEENTRY   between '" & FechaA & "' AND '" & Fechab & "' )order by a.LOTE,a.ASIENTO"
    ElseIf OptTodos Then
        ssql = ssql & " WHERE (a.DATEENTRY  between '" & FechaA & "' AND '" & Fechab & "') and a.COMPANYID='" & Empresa & "' order by BATCHID,BTCHENTRY"
    End If
End If
    'MsgBox Fechab
Debug.Print ssql
...
If rsDocumentos.EOF = False And rsDocumentos.BOF = False Then
    ...
    '**************************************************************************************************************************************************************
        ...
        .Range("C1").FormulaR1C1 = Empresa & "-Reporte Macros GL"
        .Range("F1").FormulaR1C1 = "De Fecha (DATEENTRY): " & (FechaA)
        .Range("i1").FormulaR1C1 = "A Fecha (DATEENTRY):  " & (Fechab)
'BATCHID BTCHENTRY   SRCELEDGER  SRCETYPE    FSCSYR  FSCSPERD    JRNLDESC    DATEENTRY   COMPANYID   ESTADO  "RESULTADO  "   FECHA   HORA    LOTE    ASIENTO USUARIO
'BATCHNBR    JOURNALID   TRANSNBR    ACCTID  COMPANYID   TRANSAMT    TRANSDESC   TRANSREF    TRANSDATE   "COMENTARIO "   LOTE    ASIENTO
        ...
        .Range("H3").FormulaR1C1 = "BATCHID"
        .Range("i3").FormulaR1C1 = "BTCHENTRY"
        .Range("J3").FormulaR1C1 = "TRANSNBR"
        .Range("K3").FormulaR1C1 = "SRCELEDGER"
        .Range("L3").FormulaR1C1 = "SRCETYPE"
        .Range("M3").FormulaR1C1 = "FSCSYR"
        .Range("N3").FormulaR1C1 = "FSCSPERD"
        .Range("O3").FormulaR1C1 = "JRNLDESC"
        .Range("P3").FormulaR1C1 = "DOCDATE"
        .Range("Q3").FormulaR1C1 = "DATEENTRY"
        .Range("R3").FormulaR1C1 = "BATCHDESC"
        .Range("S3").FormulaR1C1 = "ACCTID"
        .Range("T3").FormulaR1C1 = "TRANSAMT"
        .Range("U3").FormulaR1C1 = "TRANSDESC"
        .Range("V3").FormulaR1C1 = "TRANSREF"
        .Range("W3").FormulaR1C1 = "TRANSDATE"
            ...
            Do While rsDocumentos.EOF = False
                For i = 1 To 23
                    Select Case i
                        ...
                        Case 8
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!BATCHID)
                        Case 9
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!BTCHENTRY)
                        Case 10
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!TRANSNBR)
                        Case 11
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!SRCELEDGER)
                        Case 12
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!SRCETYPE)
                        Case 13
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!FSCSYR)
                        Case 14
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!FSCSPERD)
                        Case 15
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!JRNLDESC)
                        Case 16
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!DOCDATE)
                        Case 17
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!DATEENTRY)
                        Case 18
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!BATCHDESC)
                        Case 19
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!ACCTID)
                        Case 20
                            .Cells(j, i).Value = Trim(rsDocumentos!TRANSAMT)
                        Case 21
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!TRANSDESC)
                        Case 22
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!TRANSREF)
                        Case 23
                            .Cells(j, i).Value = "'" & Trim(rsDocumentos!TRANSDATE)
                        Case Else
                            'CASE ELSE
                    End Select
                Next
                rsDocumentos.MoveNext
'                Range("A" & j & ":V" & j).Select
'                Selection.Borders(xlDiagonalDown).LineStyle = xlNone
'                Selection.Borders(xlDiagonalUp).LineStyle = xlNone
'                With Selection.Borders(xlEdgeLeft)
'                    .LineStyle = xlContinuous
